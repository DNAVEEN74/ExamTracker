...
import type { Exam, TrackedExam } from '../types'
...
/** Approve exam — sets notification_verified + is_active, returns exam */
...
export async function duplicateExam(id: string, createdBy: string) {
    ...
    const { data: newExam, error } = await supabase
        ...
        .insert({
            ...
            application_end: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // placeholder
            exam_date: null,
