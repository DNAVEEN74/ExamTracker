...
import type { Exam, EligibilitySummary, EligibilityFlag, Category } from '../types'

/** Qualification ordinal mapping for comparison */
...
export async function getEligibleExams(
    ...
    try {
        // Try RPC first (when the SQL function is deployed)
        ...
        if (!rpcError && rpcData) {
            ...
            if (closing_in_days) {
                ...
                results = results.filter(
                    (e) => new Date(e.application_end) <= cutoff
                )
            }
    ...
    // ── Direct query fallback (less precise but functional) ──────────────────
    ...
    // Client-side eligibility filter (when RPC not available)
...
/** Lightweight client-side eligibility check for fallback mode */
function isEligibleClientSide(
    ...
    if (!dob) return true // Can't check without DOB — show as potentially eligible

    const ageCutoff = (exam.age_cutoff_date as string) || (exam.application_end as string)
    const ageAtCutoff =
        (Date.now() - new Date(ageCutoff).getTime()) / (1000 * 60 * 60 * 24 * 365.25) -
...
export async function getEligibilitySummary(userId: string): Promise<EligibilitySummary> {
    ...
    return {
        ...
        closing_soon: allExams.filter(
            (e) => new Date(e.application_end) <= sevenDays
        ).length,
        newly_opened: allExams.filter(
            (e) => new Date(e.created_at!) >= lastActive
        ).length,
    }
}

/** Compute an eligibility confidence flag for a single exam */
